...
以下文件/目录包含敏感信息，已在 `.gitignore` 中排除：

| 文件 | 风险 | 说明 |
|------|------|------|
| `.env` | 🔴 高 | 包含所有 API Key 和密钥 |
| `backend/wechat_openids.json` | 🔴 高 | 包含微信用户 OpenID |
| `data/` | 🟡 中 | 可能包含用户对话数据 |
| `__pycache__/` | 🟢 低 | Python 缓存文件 |

**上传前请确认**：
1. `.env` 文件不会被提交（已加入 `.gitignore`）
2. `wechat_openids.json` 不会被提交
3. 项目提供了 `.env.example` 作为配置模板，不包含真实密钥
